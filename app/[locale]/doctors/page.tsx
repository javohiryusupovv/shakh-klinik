...
export default function DoctorsPage() {
  ...
  return (
    <main className="container mx-auto px-6 py-12">
      <h1 className="text-4xl font-heading mb-8">Врачи</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filtered.map(doctor => (
          <a key={doctor.slug} href={`/doctors/${doctor.slug}`}>
            <GlassCard className="p-6 hover:scale-105 transition-transform cursor-pointer text-center">
              ...
              <p className="text-sm text-[var(--color-text-gray)]">{doctor.experienceYears} лет опыта</p>
            </GlassCard>
          </a>
        ))}
      </div>
